...
Creating functions that may make my Shiny app run clearer
```{r}
 #This is the retrieve names function. THis was created to chunk the variables of interest into meaningful groups. In the shiny app, I have created different user interfaces for various categories of interest (screening, service modality, ect.). Pulls the service names for each topic of interest in a way that is readible by the selectInput(choices = ) command. 
...
#This function is underdevelopment. In its final form, it should be able to create a stacked bar graph showing the distribution of the input across the CBSAs. 
plot_function_prop <- function(data = joined, input_spec) {
  data %>% 
