...
Nebraska_admissions_raw <- tedsa_puf_2017 %>% 
  ...
  select(CASEID, CBSA2010, SERVICES, PSOURCE, GENDER, DAYWAIT,  starts_with("SUB"), starts_with("ROUTE"), ends_with("FLG"), ALCDRUG)
...
#Coding a None dummy variable
        ...
        DAYWAIT_recoded = recode(DAYWAIT, 
          "0" = "0 days",
          "1" = "1-7 days",
          "2" = "8-14 days",
          "3" = "15-30 days",
  ...
  select(CASEID:CBSA2010_trichot, SERVICES:ALCDRUG, other_drug_use_FLG, none_FLG, DAYWAIT, DAYWAIT_recoded)
...
```{r}
daywait_across_cbsa <- 
  NE_admissions_cleaned %>% select(CBSA2010_trichot, DAYWAIT_recoded) %>% 
...
all_substance_use_summary_table <- NE_drugs_long %>% 
  count(SUBSTANCE, USAGE) %>% 
  filter(USAGE == 1) %>%
  rename(n_listing_sub_use = n) %>% 
  mutate(
  #Calculate percent of admittees using each substance
        percent = ((n_listing_sub_use / num_unique_admissions) * 100) %>% round(2),
 #Calculate rate of admittees using each substance per 100k people in state over 10
        per_100k_over_10 = ( (n_listing_sub_use / Nebraska_pop_over_10) * 100000 ) %>%  round(2)
        ) 
...
all_sub_statewide_percent_graph <- ggplot(all_substance_use_summary_table, aes(x = fct_relevel(SUBSTANCE, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ),
                 y = percent)) +
    geom_point() +
    ...
    title = "Figure 5: Percent of Admissions Listing Substance at \nAdmission to NE Treatment Centers in 2017",
...
all_sub_statewide_per_100k_10_plus_graph <- ggplot(all_substance_use_summary_table, aes(x = fct_relevel(SUBSTANCE, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ),
                 y = per_100k_over_10)) +
    geom_point() +
    ...
    title = "Figure 7: Rate of Individuals Listing Substance Use at Admission \nto NE Treatment Centers in 2017 per 100,000 Nebraskans Age \nTen and Older",
    caption = "Facilities were limited to listing at most three substances"
  ) +
    coord_flip() +
    theme_classic() 
...
all_substance_use_by_CBSA_summary_table <- 
  NE_drugs_long %>% 
...
#Computing the percent of admittees per CBSA region that reported using each substacne () 
  mutate(
    percent_of_admittees_in_stat_area_rept_sub_use = (n_listing_sub_use * 100 / n_admissions_in_stat_area) %>% round(2) ,
...
all_substance_use_by_CBSA_summary_table <- all_substance_use_by_CBSA_summary_table %>%  
#I am joining the last table with a dataframe containing the population of each CBSA region. This will serve as the denominator for computing the rate of admissions for each substance per the 100k population of the region.
  ...
  mutate(
    count_per_100k_10_plus = ((n_listing_sub_use / pop_of_CBSA_10_and_older) * 100000 ) %>% round(2),
    count_per_100k_15_plus = ((n_listing_sub_use / pop_of_CBSA_15_and_older) * 100000 ) %>% round(2)
        )
```

### Figure 6: All substances, CBSA Areas , percent
```{r all_sub_by_CBSA_percent_graph, include = F}
all_sub_by_CBSA_percent_graph <- ggplot(all_substance_use_by_CBSA_summary_table, aes(x = fct_relevel(SUBSTANCE, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ), y = percent_of_admittees_in_stat_area_rept_sub_use)) +
    geom_point() +
    ...
    x = "Substance",
    y = "Percent of Admissions",
    title = "Figure 6: Percent Substance Use at Admission to NE Treatment \nCenters in 2017 by CBSA Region"
  )+
...
all_sub_by_CBSA_per_100k_10plus_graph <- ggplot(all_substance_use_by_CBSA_summary_table, aes(x = fct_relevel(SUBSTANCE, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ), y =count_per_100k_10_plus )) +
  geom_point() +
    geom_text(aes(label = count_per_100k_10_plus) , nudge_y = 170 , size = 3) +
    geom_segment(aes(x = SUBSTANCE, 
                   xend= SUBSTANCE, 
                   y = 0, 
    ...
    x = "Substance",
    y = "Admissions per 100,000 people",
    title = "Figure 8: Rate of Individuals Listing Substance Use at Admission \nto NE Treatment Centers in 2017 per 100,000 Nebraskans \nAge Ten and Older by CBSA Region"
  )+
  facet_wrap(~ CBSA2010_trichot_labels) +
  ylim(0, 1250) +
    coord_flip() +
...
#Create variables displaying: Admissions reporting each substance as their primary substance as a percent of total admissions and a rate of admissions per 100k Nebraskans.
      mutate(
           percent = ((n_listing_sub_use / num_unique_admissions) * 100 )%>% round(2), 
           per_100k_over_10 = ((n_listing_sub_use / Nebraska_pop_over_10) * 100000) %>%  round(2),
           per_100k_over_15 = ((n_listing_sub_use / Nebraska_pop_over_15) * 100000) %>% round(2)
           ) %>% 
...
primary_sub_statewide_per_100k_10plus_graph <- ggplot(primary_substance, aes(x = fct_relevel(SUB1_recoded, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ),
                 y = per_100k_over_10)) +
    geom_point() +
...
primary_sub_statewide_percent_graph <- ggplot(primary_substance, aes(x = fct_relevel(SUB1_recoded, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ),
                 y = percent)) +
    geom_point() +
...
#compute the percent of admissions per CBSA that reported this drug as the primary substance at admission.
    percent_substance_per_CBSA =  ((n_listing_sub_use / n_admissions_in_stat_area) * 100) %>% round(2),
...
primary_sub_by_CBSA_per_100k_10plus_graph <- ggplot(primary_substance_by_CBSA, aes(x = fct_relevel(SUB1_recoded, c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol") ), y =count_per_100k_10_plus )) +
  geom_point() +
    ...
    x = "Primary Substance",
    y = "Admissions per 100,000 people",
    title = "Figure 4: Rate of Individuals Listing Primary Substance Use at \nAdmission to NE Treatment Centers in 2017 per 100,000 Nebraskans \nAge Ten and Older by CBSA Region"
  ) +
  ylim(0,1000) +
  facet_wrap(~ CBSA2010_trichot_labels)+
    coord_flip() +
...
primary_sub_by_CBSA_percent_graph <- ggplot(primary_substance_by_CBSA, aes(x = fct_relevel(SUB1_recoded,c("None", "Other", "Benzodiazepine","Heroin", "Cocaine/crack","Synthetic opioids", "Marijuana", "Methamphetamine" , "Alcohol")), y = percent_substance_per_CBSA)) +
  geom_point() +
...
Hmisc::label(dataset_with_amt_drugs_mentioned$num_drugs_mentioned) <- "Number of Drugs Listed at Admission"
...
incarceration_ref_by_subsatance_data <- NE_drugs_long %>% filter(USAGE == 1) %>% 
     mutate(PSOURCE_mod = recode(PSOURCE, 
       "Alcohol/drug use care provider" = "Other Orgnizational Referral", 
       "School" = "Other Orgnizational Referral",
       "Other Community Referral" = "Other Orgnizational Referral",
       "Employer/EAP" = "Other Orgnizational Referral",
       "Other health care provider" = "Other Orgnizational Referral",
       "Court/criminal justice referral/DUI/DWI" = "Criminal Justice System Referal"
)) %>% 
  filter(PSOURCE_mod != "NA")
       
Hmisc::label(incarceration_ref_by_subsatance_data$PSOURCE_mod) <- "Referral Source"
...
# Referal Sources Across CBSA
```{r}
NE_admissions_cleaned %>% 
      mutate(PSOURCE_mod = recode(PSOURCE, 
       "Alcohol/drug use care provider" = "Other Orgnizational Referral", 
       "School" = "Other Orgnizational Referral",
       "Other Community Referral" = "Other Orgnizational Referral",
    ...
    select(PSOURCE_mod, CBSA2010_trichot) %>% 
    tbl_summary(by = CBSA2010_trichot) %>% 
    as_gt()
  

incarceration_ref_by_subsatance_data %>% count(CASEID)
...
```{r}
joined <- inner_join(tedsa_puf_2017, tedsd_puf_2017, by = "CASEID")



tedsd_cleaned <- tedsd_puf_2017 %>% 
  filter(STFIPS == 31) %>% 
  mutate(
 CASEID = factor(CASEID),
 CBSA2010_trichot = recode(CBSA2010,
  ...
  select(
    CASEID, LOS, DAYWAIT, CBSA2010, CBSA2010_trichot
  )
...
tedsd_cleaned %>% 
  select(CBSA2010_trichot, DAYWAIT) %>%  
  tbl_summary(
              by = CBSA2010_trichot
            )
...
test <- left_join(NE_admissions_cleaned, tesd_cleaned, by = "CASEID")

test %>% mutate(
  log_vec = ifelse(CBSA2010.x == CBSA2010.y, T, F)
)
